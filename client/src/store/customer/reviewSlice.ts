...
import type { Review, ReviewState } from "../../types/customer/reviewTypes";
...
export const { setReviews, setStatus, editReview, deleteReviewById } =
  reviewSlice.actions;
...
export function addReview(payload: { productId: string; data: FormData }) {
  return async function addReviewThunk(dispatch: AppDispatch) {
    dispatch(setStatus(Status.LOADING));
    try {
      await APIAuthenticated.post(
        `/customer/review/product/${payload.productId}`,
        payload.data,
      );
